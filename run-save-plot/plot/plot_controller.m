...
function fig = plotControllerDashboard(data, tlim)
    ...
    ax = nexttile(layout);
    plot_signal_group(ax, {phiTarget, phi}, ["phi\_target", "phi"], ...
        "Roll Angle", "rad", tlim);

    ax = nexttile(layout);
    plot_signal_group(ax, {cmd, canardAngle}, ["cmd", "canard\_angle"], ...
        "Canard Angle", "deg", tlim);

    ax = nexttile(layout);
    plot_signal_group(ax, {wTarget, w}, ["w\_target", "w"], ...
        "Angular Rates", "rad/s", tlim);

    ax = nexttile(layout);
    plot_signal_group(ax, {CL, CLEst}, ["CL", "CL\_est"], ...
