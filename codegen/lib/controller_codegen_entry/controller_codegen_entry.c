...
void controller_codegen_entry(double b_time, const double xR[2], double pdyn,
                              double delta, double *u, double *r,
                              double *C_l_delta)
{
  ...
  /*  regression  */
  b_r = _mm_loadu_pd(&P_minus[0]);
  r1 = _mm_loadu_pd(&Q[0]);
  _mm_storeu_pd(&Q[0], _mm_add_pd(b_r, r1));
  b_r = _mm_loadu_pd(&P_minus[2]);
  r1 = _mm_loadu_pd(&Q[2]);
  _mm_storeu_pd(&Q[2], _mm_add_pd(b_r, r1));
  ...
  eye(P_minus);
  c[0] += K[0] * L_delta;
  b_r = _mm_loadu_pd(&K[0]);
  r1 = _mm_loadu_pd(&P_minus[0]);
  r2 = _mm_set1_pd(0.0);
  _mm_storeu_pd(
      &dv[0],
      _mm_sub_pd(r1, _mm_add_pd(r2, _mm_mul_pd(b_r, _mm_set1_pd(r_idx_0)))));
  c[1] += K[1] * L_delta;
  b_r = _mm_loadu_pd(&K[0]);
  ...
  _mm_storeu_pd(
      &dv[2],
      _mm_sub_pd(r1,
                 _mm_add_pd(r2, _mm_mul_pd(b_r, _mm_set1_pd(pdyn_params)))));
  memset(&P_minus[0], 0, sizeof(double) << 2);
  b_r = _mm_loadu_pd(&dv[0]);
  r1 = _mm_loadu_pd(&P_minus[0]);
  _mm_storeu_pd(&P_minus[0],
                _mm_add_pd(r1, _mm_mul_pd(b_r, _mm_set1_pd(Q[0]))));
  b_r = _mm_loadu_pd(&dv[2]);
  r1 = _mm_loadu_pd(&P_minus[0]);
  _mm_storeu_pd(&P_minus[0],
                _mm_add_pd(r1, _mm_mul_pd(b_r, _mm_set1_pd(Q[1]))));
  b_r = _mm_loadu_pd(&dv[0]);
  r1 = _mm_loadu_pd(&P_minus[2]);
  _mm_storeu_pd(&P_minus[2],
                _mm_add_pd(r1, _mm_mul_pd(b_r, _mm_set1_pd(Q[2]))));
  b_r = _mm_loadu_pd(&dv[2]);
  r1 = _mm_loadu_pd(&P_minus[2]);
  _mm_storeu_pd(&P_minus[2],
                _mm_add_pd(r1, _mm_mul_pd(b_r, _mm_set1_pd(Q[3]))));
  memset(&dv1[0], 0, sizeof(double) << 2);
  b_r = _mm_loadu_pd(&P_minus[0]);
  r1 = _mm_loadu_pd(&dv1[0]);
  _mm_storeu_pd(&dv1[0], _mm_add_pd(r1, _mm_mul_pd(b_r, _mm_set1_pd(dv[0]))));
  Q[0] = K[0] * K[0];
  b_r = _mm_loadu_pd(&P_minus[2]);
  r1 = _mm_loadu_pd(&dv1[0]);
  _mm_storeu_pd(&dv1[0], _mm_add_pd(r1, _mm_mul_pd(b_r, _mm_set1_pd(dv[2]))));
  L_delta = K[0] * K[1];
  Q[1] = L_delta;
  b_r = _mm_loadu_pd(&P_minus[0]);
  r1 = _mm_loadu_pd(&dv1[2]);
  _mm_storeu_pd(&dv1[2], _mm_add_pd(r1, _mm_mul_pd(b_r, _mm_set1_pd(dv[1]))));
  Q[2] = L_delta;
  b_r = _mm_loadu_pd(&P_minus[2]);
  r1 = _mm_loadu_pd(&dv1[2]);
  _mm_storeu_pd(&dv1[2], _mm_add_pd(r1, _mm_mul_pd(b_r, _mm_set1_pd(dv[3]))));
  Q[3] = K[1] * K[1];
  b_r = _mm_loadu_pd(&dv1[0]);
  r1 = _mm_loadu_pd(&Q[0]);
  _mm_storeu_pd(&P_minus[0], _mm_add_pd(b_r, r1));
  b_r = _mm_loadu_pd(&dv1[2]);
  r1 = _mm_loadu_pd(&Q[2]);
  _mm_storeu_pd(&P_minus[2], _mm_add_pd(b_r, r1));
...
/* End of code generation (controller_codegen_entry.c) */
