                              ...
                              double *C_l_delta)
{
  ...
  /*  regression  */
  c_r = _mm_loadu_pd(&P_minus[0]);
  r1 = _mm_loadu_pd(&Q[0]);
  _mm_storeu_pd(&Q[0], _mm_add_pd(c_r, r1));
  c_r = _mm_loadu_pd(&P_minus[2]);
  r1 = _mm_loadu_pd(&Q[2]);
  _mm_storeu_pd(&Q[2], _mm_add_pd(c_r, r1));
  /*  covariance prediction */
  memset(&K[0], 0, sizeof(double) << 1);
  L_delta = r_idx_0 * Q[0];
  b_tmp_tmp = pdyn_params * Q[3];
  d_r = ((K[0] + L_delta) + pdyn_params * Q[1]) * r_idx_0 +
        ((K[1] + r_idx_0 * Q[2]) + b_tmp_tmp) * pdyn_params;
  /*  correction gain. the stuff inside in brackets is just a scalar so you can
   * just divide */
  K[0] = (L_delta + Q[2] * pdyn_params) / (d_r + 1.0);
  K[1] = (Q[1] * r_idx_0 + b_tmp_tmp) / (d_r + 1.0);
  L_delta = w_dot_old - (r_idx_0 * c[0] + pdyn_params * c[1]);
  /*  coefficient correction */
  eye(P_minus);
  c[0] += K[0] * L_delta;
  c_r = _mm_loadu_pd(&K[0]);
  r1 = _mm_loadu_pd(&P_minus[0]);
  r2 = _mm_set1_pd(0.0);
  _mm_storeu_pd(
      &dv[0],
      _mm_sub_pd(r1, _mm_add_pd(r2, _mm_mul_pd(c_r, _mm_set1_pd(r_idx_0)))));
  c[1] += K[1] * L_delta;
  c_r = _mm_loadu_pd(&K[0]);
  r1 = _mm_loadu_pd(&P_minus[2]);
  _mm_storeu_pd(
      &dv[2],
      _mm_sub_pd(r1,
                 _mm_add_pd(r2, _mm_mul_pd(c_r, _mm_set1_pd(pdyn_params)))));
  memset(&P_minus[0], 0, sizeof(double) << 2);
  c_r = _mm_loadu_pd(&dv[0]);
  r1 = _mm_loadu_pd(&P_minus[0]);
  _mm_storeu_pd(&P_minus[0],
                _mm_add_pd(r1, _mm_mul_pd(c_r, _mm_set1_pd(Q[0]))));
  c_r = _mm_loadu_pd(&dv[2]);
  r1 = _mm_loadu_pd(&P_minus[0]);
  _mm_storeu_pd(&P_minus[0],
                _mm_add_pd(r1, _mm_mul_pd(c_r, _mm_set1_pd(Q[1]))));
  c_r = _mm_loadu_pd(&dv[0]);
  r1 = _mm_loadu_pd(&P_minus[2]);
  _mm_storeu_pd(&P_minus[2],
                _mm_add_pd(r1, _mm_mul_pd(c_r, _mm_set1_pd(Q[2]))));
  c_r = _mm_loadu_pd(&dv[2]);
  r1 = _mm_loadu_pd(&P_minus[2]);
  _mm_storeu_pd(&P_minus[2],
                _mm_add_pd(r1, _mm_mul_pd(c_r, _mm_set1_pd(Q[3]))));
  memset(&dv1[0], 0, sizeof(double) << 2);
  c_r = _mm_loadu_pd(&P_minus[0]);
  r1 = _mm_loadu_pd(&dv1[0]);
  _mm_storeu_pd(&dv1[0], _mm_add_pd(r1, _mm_mul_pd(c_r, _mm_set1_pd(dv[0]))));
  Q[0] = K[0] * K[0];
  c_r = _mm_loadu_pd(&P_minus[2]);
  r1 = _mm_loadu_pd(&dv1[0]);
  _mm_storeu_pd(&dv1[0], _mm_add_pd(r1, _mm_mul_pd(c_r, _mm_set1_pd(dv[2]))));
  L_delta = K[0] * K[1];
  Q[1] = L_delta;
  c_r = _mm_loadu_pd(&P_minus[0]);
  r1 = _mm_loadu_pd(&dv1[2]);
  _mm_storeu_pd(&dv1[2], _mm_add_pd(r1, _mm_mul_pd(c_r, _mm_set1_pd(dv[1]))));
  Q[2] = L_delta;
  c_r = _mm_loadu_pd(&P_minus[2]);
  r1 = _mm_loadu_pd(&dv1[2]);
  _mm_storeu_pd(&dv1[2], _mm_add_pd(r1, _mm_mul_pd(c_r, _mm_set1_pd(dv[3]))));
  Q[3] = K[1] * K[1];
  c_r = _mm_loadu_pd(&dv1[0]);
  r1 = _mm_loadu_pd(&Q[0]);
  _mm_storeu_pd(&P_minus[0], _mm_add_pd(c_r, r1));
  c_r = _mm_loadu_pd(&dv1[2]);
  r1 = _mm_loadu_pd(&Q[2]);
  _mm_storeu_pd(&P_minus[2], _mm_add_pd(c_r, r1));
  ...
  *u = fmin(
      fmax((K[0] * xR[0] +
            L_delta * sqrt(2.0 * d_r + (b_tmp_tmp + r_idx_0 * 20.0)) * xR[1]) +
               -K[0] * *b_r,
           -0.17453292519943295),
      0.17453292519943295);
  /*  upper bounds */
