function [x, P] = flight_filter(dt, x, P, bias, board_accel, board_gyro, mti_accel, mti_gyro, ad_accel, ad_gyro, board_baro, board_mag, mti_baro, mti_mag)
    ...
    if board_baro.baro_status == 1 % only correct with alive IMUs
        %%% y = [ P(1) ]
        R = 1;
        [xhat, Phat] = ekf_correct(@meas_baro, @meas_baro_jacobian, x, P, board_baro.baro_meas, bias.board_baro, R);
        x = xhat; P = Phat;
    end
    if mti_baro.baro_status == 1 % only correct with alive IMUs
        %%% y = [ P(1) ]
        R = 1;
        [xhat, Phat] = ekf_correct(@meas_baro, @meas_baro_jacobian, x, P, mti_baro.baro_meas, bias.mti_baro, R);
        x = xhat; P = Phat;
    end
    ...
    if board_mag.mag_status == 1
        %%% y = [  Mag(3) ]
        R = diag([1, 1, 1])*0.01;
        [xhat, Phat] = ekf_correct(@meas_mag, @meas_mag_jacobian, x, P, board_mag.mag_meas, bias.board_mag, R);
        x = xhat; P = Phat;
    end 
    if mti_mag.mag_status == 1
        %%% y = [  Mag(3) ]
        R = diag([1, 1, 1])*0.01;
        [xhat, Phat] = ekf_correct(@meas_mag, @meas_mag_jacobian, x, P, mti_mag.mag_meas, bias.mti_mag, R);
        x = xhat; P = Phat;
    end 
